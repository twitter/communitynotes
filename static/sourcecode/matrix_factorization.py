...
class BiasedMatrixFactorization(torch.nn.Module):
  ...
  def __init__(
    ...
    self.user_factors = torch.nn.Embedding(n_users, n_factors, sparse=False, device=self.device)
    self.item_factors = torch.nn.Embedding(n_items, n_factors, sparse=False, device=self.device)
    self.user_intercepts = torch.nn.Embedding(n_users, 1, sparse=False, device=self.device)
    self.item_intercepts = torch.nn.Embedding(n_items, 1, sparse=False, device=self.device)
    self.use_global_intercept = use_global_intercept
    self.global_intercept = torch.nn.parameter.Parameter(torch.zeros(1, 1, device=self.device))
...
def initialize_parameters(
  ...
  if noteInit is not None:
    ...
    mf_model.item_intercepts.weight.data = torch.tensor(
      np.expand_dims(noteInit[c.noteInterceptKey].astype(np.float32).values, axis=1), device=mf_model.device,
    )
  ...
  if globalInterceptInit is not None:
    ...
    mf_model.global_intercept = torch.nn.parameter.Parameter(torch.ones(1, 1, device=mf_model.device) * globalInterceptInit)
