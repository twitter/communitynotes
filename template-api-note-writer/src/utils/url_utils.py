...
def extract_and_validate_urls(text: str, citations: list[str], timeout: int = 10) -> list[tuple[str, int | None]]:
    """
    ...
    Example:
        ...
        >>> failed_urls = extract_and_validate_urls(text)
        >>> if failed_urls:
    ...
    for url in urls:
        if url in citations:
            continue
        is_valid, status_code = validate_url(url, timeout=timeout)
        if not is_valid:
...
def validate_url(url: str, timeout: int = 10) -> tuple[bool, int | None]:
    """
    ...
    Example:
        >>> is_valid, status_code = validate_url("https://example.com")
        >>> if is_valid:
